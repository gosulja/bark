...
pub fn decode(bytes: &[u8], addr: u64) -> Option<Instruction> {
    ...
    */
    ...
    if (0xB8..=0xBF).contains(&opc) {
        ...
        let imm = match reg_size {
            64 => u64::from_le_bytes(bytes[idx..need].try_into().unwrap()),
            _ => u32::from_le_bytes(bytes[idx..need].try_into().unwrap()) as u64,
        };
    ...
    if (0xB0..=0xB7).contains(&opc) {
        ...
        if bytes.len() < idx + 1 { return None; }
