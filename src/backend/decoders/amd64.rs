...
const REG8: &[&str] = &["al", "cl", "dl", "bl", "ah", "ch", "dh", "bh", "r8b", "r9b", "r10b", "r11b", "r12b", "r13b", "r14b", "r15b"];
const REG16: &[&str] = &["ax", "cx", "dx", "bx", "sp", "bp", "si", "di", "r8w", "r9w", "r10w", "r11w", "r12w", "r13w", "r14w", "r15w"];
const REG32: &[&str] = &["eax", "ecx", "edx", "ebx", "esp", "ebp", "esi", "edi", "r8d", "r9d", "r10d", "r11d", "r12d", "r13d", "r14d", "r15d"];
const REG64: &[&str] = &["rax", "rcx", "rdx", "rbx", "rsp", "rbp", "rsi", "rdi", "r8", "r9", "r10", "r11", "r12", "r13", "r14", "r15"];
...
impl VariableRecovery {
    ...
    pub fn process_operand(&mut self, operand: &str, addr: u64) -> String {
        ...
        if REG64.contains(&operand) || REG32.contains(&operand) || 
           REG16.contains(&operand) || REG8.contains(&operand) {
    ...
    fn process_memory_operand(&mut self, operand: &str) -> String {
        if operand.contains("rbp") || operand.contains("ebp") {
            if let Some(offset_str) = operand.strip_prefix("[rbp").or_else(|| operand.strip_prefix("[ebp")) {
    ...
    fn decode_modrm(&mut self, prefixes: &Prefixes) -> Option<(String, usize)> {
        ...
        match mode {
            0b11 => {
                ...
                Some((reg_name.to_string(), 0))
            },
            0b00 | 0b01 | 0b10 => {
                let disp_size = match mode {
                    0b00 => if rm == 5 { 4 } else { 0 },
                    0b01 => 1,
                    0b10 => 4,
                    _ => unreachable!(),
                };
                ...
                if rm == 4 {
                    ...
                    let base_reg = if rm == 5 && mode == 0 {
                        ...
                        self.get_register(rm as usize, 64, prefixes.rex_b)?.to_string()
                    };
...
pub fn decode(bytes: &[u8], addr: u64) -> Option<Instruction> {
    ...
    match opc {
        ...
        0x01 | 0x09 | 0x11 | 0x19 | 0x21 | 0x29 | 0x31 | 0x39 => decode_alu_rm_reg(&mut cursor, opc, &prefixes, addr),
        _ => None,
    }
}

fn decode_mov_reg_imm(cursor: &mut Cursor, opc: u8, prefixes: &Prefixes, addr: u64) -> Option<Instruction> {
...
fn decode_mov_rm_imm(cursor: &mut Cursor, opc: u8, prefixes: &Prefixes, addr: u64) -> Option<Instruction> {
...
fn decode_mov_reg_rm(cursor: &mut Cursor, opc: u8, prefixes: &Prefixes, addr: u64) -> Option<Instruction> {
...
fn decode_alu_rm_imm(cursor: &mut Cursor, opc: u8, prefixes: &Prefixes, addr: u64) -> Option<Instruction> {
    let modrm = cursor.peek_u8()?;
...
fn decode_alu_rm_reg(cursor: &mut Cursor, opc: u8, prefixes: &Prefixes, addr: u64) -> Option<Instruction> {
...
fn decode_push_pop(cursor: &mut Cursor, opc: u8, prefixes: &Prefixes, addr: u64) -> Option<Instruction> {
    let is_pop = opc >= 0x58;
    let reg_id = (opc & 0x07) as usize;
    let (_, reg_size) = cursor.get_reg_size(prefixes);
